...
function main() {
  try {
    ...
    var reader = new tc.Reader(input);
        var treeCount = reader.next('int'); reader.next();
    var width = reader.next('int'); reader.next();
    var height = reader.next('int'); reader.next();
    var distance = reader.next('int');

    var BackyardTrees = require("./BackyardTrees");
    var result = BackyardTrees.countWays(treeCount, width, height, distance);
